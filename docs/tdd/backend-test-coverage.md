...
All tests use transaction rollback for automatic cleanup:

```csharp
[TestInitialize]
public void Setup()
{
    InitializeWithTransaction();  // Starts transaction
...
{
    _service?.Dispose();
    CleanupWithTransaction();  // Rolls back transaction
}
```

### Entity State Management

Services create their own DbContext, so test data must be detached:

```csharp
// After creating test data, detach so service's context can load fresh
DetachAllEntities(_context);
```

### Verification with Fresh Context

Always verify database state with a fresh context:
...
dotnet test DomainTest/DomainTest.csproj
```
