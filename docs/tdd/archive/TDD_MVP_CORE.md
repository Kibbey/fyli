# Technical Design Document: MVP Core — Capture & Share
...
┌─────────────────────────────────────────────────┐
│              Vue 3 Frontend (NEW)               │
│   Composition API, TypeScript, Pinia, Bootstrap │
└───────────────────┬─────────────────────────────┘
                    │ HTTPS / JSON
┌───────────────────▼─────────────────────────────┐
│           C#/.NET Backend (EXISTING)            │
│                                                 │
│  ┌───────────────────────────────────────── ┐   │
│  │ EXISTING: UserController, DropController,│   │
│  │ CommentController, ConnectionController, │   │
│  │ ImageController, MovieController         │   │
│  ├──────────────────────────────────────── ─┤   │
│  │ NEW: ShareLinkController                 │   │
│  └───────────────────────────────────────── ┘   │
│                                                 │
...
## Phase 1: Backend — Shareable Memory Links
...
**No raw SQL is written for schema changes.** The SQL below is shown for reference only — EF Core generates it from the entity and configuration.

New table: `MemoryShareLinks` (generated from entity below)

```sql
-- Reference only — generated by EF Core from the POCO entity + StreamContext config
...
**Design decisions:**
- Separate table from `ShareRequests` — that system handles user-to-user connection invitations with email matching, group sharing, and bidirectional connection creation. Memory share links are simpler: one token → one memory → anonymous read access.
- `ExpiresAt` nullable — links don't expire by default, but the creator can revoke (set `IsActive = false`).
...
| `RegisterAndConnectAsync(Guid token, string email, string name, bool acceptTerms)` | **Delegates to existing services** — does NOT reimplement registration. Calls `UserService` for account creation (preserving existing behavior: hello-world content, welcome emails, terms acceptance) and `SharingService` for bidirectional `UserUser` connection creation. Returns JWT via existing `UserWebToken.generateJwtToken`. If user already exists, skips registration and only creates connection. If already connected, skips connection creation. |
...
**Token security:** UUIDs (v4) have 122 bits of randomness — enumeration is not practically feasible, but rate limiting provides defense in depth.

### 1.7 Sharing Private Memories

A user **can** generate a share link for a memory marked as "private." The link grants read-only access to anyone with the token regardless of privacy setting. Privacy controls who sees the memory in the authenticated stream; share links are an explicit act of sharing that overrides stream visibility. This is intentional — a parent may want a memory private from their connection feed but still shareable via a direct link.

### 1.8 Permanent Drop Access via Share Link

When a user interacts with a share link while authenticated (or becomes authenticated through it), they receive **permanent access** to the shared drop. This means the drop appears in their normal stream via the existing `GetAllDrops` query — no token needed for subsequent views.
...
/// Idempotent — safe to call multiple times for the same user/drop.
/// </summary>
...
/// Idempotent — no-ops if connection already present.
/// </summary>
...
**For already-authenticated users:** The frontend detects a valid JWT when loading `/s/:token` and automatically calls `POST /api/sharelinks/{token}/claim` — no extra user action needed.
...
User arrives at /s/:token
  │
  ├─ Has JWT? ──yes──→ GET /api/sharelinks/{token} (view memory)
  │                     POST /api/sharelinks/{token}/claim (grant permanent access)
  │                     Show memory + comment input
  │
  └─ No JWT? ───────→ GET /api/sharelinks/{token} (view memory read-only)
                       Show memory + registration/sign-in CTA
                         │
                         ├─ Registers → POST /api/sharelinks/{token}/register
                         │              → JWT returned, access granted, can comment
                         │
                         └─ Signs in  → POST /api/sharelinks/{token}/signin (triggers magic link)
                                       → User clicks magic link → /auth/verify/:token
                                       → Frontend redirects back to /s/:token
                                       → POST /api/sharelinks/{token}/claim (grant access)
```
...
## Phase 2: Frontend — Project Setup & Auth
...
│   │   ├── memory/              # Memory card, creation form
│   │   └── comment/             # Comment list, comment form
│   ├── composables/
│   │   ├── useAuth.ts           # Auth state & magic link flow
│   │   └── useApi.ts            # Axios wrapper with JWT
│   ├── router/
│   │   └── index.ts
│   ├── services/
│   │   ├── authApi.ts           # /api/users endpoints
│   │   ├── memoryApi.ts         # /api/drops endpoints
│   │   ├── mediaApi.ts          # /api/images, /api/movies endpoints
│   │   ├── commentApi.ts        # /api/comments endpoints
│   │   ├── shareLinkApi.ts      # /api/sharelinks endpoints
│   │   └── connectionApi.ts     # /api/connections endpoints
│   ├── stores/
│   │   ├── auth.ts              # User session, JWT
│   │   └── stream.ts            # Memory feed state
│   ├── types/
...
│   │   │   └── StreamView.vue
│   │   ├── memory/
│   │   │   ├── CreateMemoryView.vue
│   │   │   └── MemoryDetailView.vue
│   │   ├── share/
│   │   │   └── SharedMemoryView.vue    # Public: no auth required
│   │   └── onboarding/
│   │       ├── WelcomeView.vue
│   │       ├── FirstMemoryView.vue
│   │       └── FirstShareView.vue
...
| PWA | vite-plugin-pwa | Deferred — not in MVP scope. Add `manifest.json` placeholder only for future enablement |
...
```
Register:
  RegisterView → POST /api/users/register → "Check your email" screen

Login:
  LoginView → POST /api/users/token → "Check your email" screen

Magic Link:
  User clicks email link → /auth/verify/:token → POST /api/users/login
  → JWT stored in localStorage → redirect to / (or onboarding if new user)

Share Link Registration:
  SharedMemoryView → "Sign up to comment" CTA → inline registration form
  → POST /api/sharelinks/{token}/register → JWT stored → auto-connected
  → redirect back to SharedMemoryView (now authenticated, can comment)
```
...
## Phase 3: Frontend — Memory Stream & Creation
...
**Actions:**
- `fetchPage()` — `GET /api/drops?page={page}` → append to memories, increment page
- `reset()` — clear and reload from page 0

### 3.2 StreamView

- Renders list of `MemoryCard` components
- Infinite scroll: IntersectionObserver on sentinel element at bottom
- Floating "+" button → navigates to `/memory/new`
...
```
┌─────────────────────────────────┐
│ [Avatar] Creator Name    Date   │
│                                 │
│ Description text...             │
│                                 │
│ [Photo grid / Video player]     │
│                                 │
│ 💬 3 comments     [Share]       │
└─────────────────────────────────┘
```

- Tap card → navigate to `/memory/:id` (full detail + comments)
- Share button → calls `POST /api/sharelinks/{dropId}` → copies link to clipboard → toast confirmation
...
- Video upload: `POST /api/movies/upload/request` → direct S3 upload → `POST /api/movies/upload/complete`
- Video limit: 500 MB (client-side validation before upload)
- Privacy toggle: binary private/shared
  - **Private:** `POST /api/drops` with empty `TagDrops` array — no groups attached, only creator can see it in the stream
...
- Menu (⋮): Edit, Delete, Share link
- Edit → inline editing of description, date, privacy, media
- Delete → confirmation dialog → `DELETE /api/drops/{id}`
- Comments loaded via `GET /api/comments/{dropId}`
- Add comment: `POST /api/comments` with optional photo

---

## Phase 4: Frontend — Shareable Links & Conversion
...
```
Register → WelcomeView → FirstMemoryView → FirstShareView → StreamView
```
...
│   It can be anything — a photo  │
│   from today, a favorite memory,│
│   something your kid said.      │
│                                 │
│   [CreateMemoryView embedded]   │
│                                 │
│         skip →                  │
└─────────────────────────────────┘
```

After save → auto-advance to FirstShareView.
...
After share (or skip) → redirect to `/` (StreamView). Set `isNewUser = false`.

---

## Phase 6: Frontend — Email Invitations (Deprioritized)
...
- Email input → `POST /api/connections` with email and alias
- Confirmation toast
...
| CreateLink_ExistingActive_ReturnsSame | Unit | Idempotent — returns existing token |
...
| RegisterAndConnect_ExistingUser_ConnectsOnly | Unit | Existing email → just connect |
| RegisterAndConnect_AlreadyConnected_NoopConnection | Unit | Users already connected → skip connection, return JWT |
| RegisterAndConnect_MissingTerms_Throws | Unit | acceptTerms=false → validation error |
| GetMemoryByToken_Expired_Returns404 | Unit | ExpiresAt in the past → not found |
...
| Onboarding flow navigation | E2E | Register → welcome → first memory → share → stream |
...
| **Phase 5** | Frontend: onboarding (welcome → first memory → first share) | Phase 3 + Phase 4 |
...
*Created: 2026-01-31*
*Updated: 2026-01-31 — v1.2: Added permanent drop access via share link (section 1.8), /claim and /signin endpoints, private drop handling, EnsureConnectionAsync, batched saves, over-sharing and deactivation behavior documented*
*Previous: v1.1 — Addressed code review (service delegation, rate limiting, terms acceptance, CORS, error states, privacy clarification, missing tests, dependency corrections)*
*Status: Draft*
